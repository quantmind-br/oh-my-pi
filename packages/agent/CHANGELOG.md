...
- Fixed aborting a Cursor turn while an async `cursorOnToolResult` transformer was pending. Both the normal message drain and provider-error drain could wait forever on a host hook, and a late transformer could patch detached state after the run ended. The request signal is now forwarded to the hook, abort abandons the run-scoped result buffer immediately, and late completions cannot leak into the next prompt.
