...
- Fixed Cursor cancellation hanging behind an in-flight exec handler or `onToolResult` hook. The request signal is now threaded through every Cursor exec/MCP dispatch and result transformer, each wait races that signal, and the terminal barrier is skipped only for actual cancellation. Ordinary transport failures still wait for tools that may already have side effects so their real results remain paired.
- Fixed Cursor provider shell streaming writing to a closed HTTP/2 request after cancellation. A non-cooperative `shellStream` handler could leave its 100 ms stdout/stderr flush timers and callbacks alive when the abort race rejected, because normal completion cleanup was skipped. Abort now invalidates every stream callback, and both timers and buffered output are cleared unconditionally in `finally`.
